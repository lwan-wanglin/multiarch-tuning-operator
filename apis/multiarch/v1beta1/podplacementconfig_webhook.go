...
func (v *PodPlacementConfigValidator) validate(ctx context.Context, obj runtime.Object) (warnings admission.Warnings, err error) {
	...
	err = v.Get(ctx, client.ObjectKey{Name: common.SingletonResourceObjectName}, clusterPodPlacementConfig)
	if apierrors.IsNotFound(err) {
		...
		return nil, fmt.Errorf("failed to get ClusterPodPlacementConfig: %w", err)
	}

	ppcs := &PodPlacementConfigList{}
	if err := v.List(ctx, ppcs, client.InNamespace(ppcNew.Namespace)); err != nil {
		return nil, fmt.Errorf("failed to list PodPlacementConfig in namespace %s: %w", ppcNew.Namespace, err)
	}
